...
export default function Booking() {
  ...
  function isSlotBooked(slot) {
    ...
    // ถ้า end time คือ 00:00 แสดงว่าต้องข้ามวัน (23:30 - 00:00)
    if (slotEnd.getTime() <= slotStart.getTime()) {
      slotEnd.setDate(slotEnd.getDate() + 1); // เพิ่มวันให้กับ end time
    }

    // ตรวจสอบว่าช่วงเวลาที่เลือกชนกับช่วงเวลาที่จองไว้หรือไม่
    for (let booking of bookedSlots) {
      ...
      // ถ้า end time ของการจองเป็น 00:00 ต้องข้ามวันเช่นกัน
      if (bookingEnd.getTime() <= bookingStart.getTime()) {
        bookingEnd.setDate(bookingEnd.getDate() + 1);
      }

      // ตรวจสอบว่าเวลาที่เลือกทับกับการจองหรือไม่
      if (
        slotStart.getTime() < bookingEnd.getTime() &&
        slotEnd.getTime() > bookingStart.getTime()
      ) {
        return true; // ถ้าทับกันให้คืนค่า true ว่าถูกจองแล้ว
      }
    }

    return false; // ถ้าไม่ทับให้คืนค่า false
  }
  ...
  function calculateSelectedTimes() {
    ...
    if (endHour === 0 && endMinute === 0) {
      ...
      alert("ไม่สามารถจองข้ามวันได้ กรุณาเลือกเวลาใหม่");
      setSelectedSlots([]);
      setTimeStart("");
      setTimeEnd("");
      setTotalHours(0);
  ...
  const handleSubmit = async () => {
    ...
    if (priceDeposit > 0) {
      if (!depositSlip) {
        alert("กรุณาแนบสลิปหลักฐานการชำระเงินมัดจำก่อนทำการจอง");
        return;
      }
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setBookingStatus(`Error: ${errorData.message || "Unknown error"}`);
        alert(`Error: ${errorData.message || "Unknown error"} - Status Code: ${response.status}`);
      } else {
        const data = await response.json();
        if (data.success) {
          // Fetch the updated booking status after successful submission
          await fetchBookingStatus(data.bookingId); // Pass the booking ID or any identifier you have
          alert("✅ บันทึกการจองสำเร็จ");
        } else {
          setBookingStatus(`Error: ${data.message || "Unknown error"}`);
          alert(`Error: ${data.message || "Unknown error"}`);
        }
      }
    } catch (error) {
      setBookingStatus(`Error: ${error.message || "Unknown error"}`);
      alert(`เกิดข้อผิดพลาด: ${error.message || "Unknown error"}`);
    }
  };
  
  // Fetch the updated booking status from the backend
  const fetchBookingStatus = async (bookingId) => {
    try {
      ...
      if (data.success) {
        ...
        setBookingStatus(data.status); // Assume the API returns a "status" field
      } else {
          ...
          <div className="slots-grid">
            {slots.map((slot, index) => {
              ...
              return (
                <div
                  key={index}
                  className={`slot-box ${isSelected ? "selected-slot" : ""} ${
