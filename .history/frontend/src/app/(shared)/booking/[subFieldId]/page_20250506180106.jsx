...
export default function Booking() {
  ...
  return (
    <div>
      ...
      <h1 className="sub-header">เลือกช่วงเวลา</h1>
  ...
  <h3 className="addon-title">เลือกราคาเสริม</h3>
  <div className="addon-grid">
    <div
      className={`addon-card ${selectPrice === "subFieldPrice" ? "selected" : ""}`}
      onClick={() => handlePriceOnChange({ target: { value: "subFieldPrice" } })}
    >
    ...
    {addOns.map((addOn) => (
      <div
        key={addOn.add_on_id}
        className={`addon-card ${selectPrice === addOn.add_on_id ? "selected" : ""}`}
        onClick={() => handlePriceOnChange({ target: { value: addOn.add_on_id } })}
      >
        ...
        <div className="book-sider">
          <div className="book-sum-box">
            ...
            {subFieldData !== "ไม่พบข้อมูล" ? (
              <h2 className="sub-field-title">{subFieldData.sub_field_name}</h2>
            ) : (
