...
export default function Booking() {
  ...
  function isSlotAvailable(slot) {
    ...
    for (let booking of bookedSlots) {
      ...
      const bookingStart = new Date(1970, 0, 1, bookStartHour, bookStartMinute);
      let bookingEnd = new Date(1970, 0, 1, bookEndHour, bookEndMinute);
      if (bookingEnd <= bookingStart) bookingEnd.setDate(bookingEnd.getDate() + 1);
  
      if (slotStart < bookingEnd && slotEnd > bookingStart) {
        return false; // ทับเวลา
      }
    }
          ...
          <div className="book-content">
            <div className="slots-grid">
              {slots.map((slot, index) => {
                ...
                const isAvailable = isSlotAvailable(slot); // ฟังก์ชันใหม่ที่เช็กชนกับ booking ทุกรายการ

                return (

                  <div
                    key={index}
                    className={`${slotClass} ${!isAvailable ? "unavailable-slot" : ""}`}
                    onClick={() => {
                      if (isAvailable) toggleSelectSlot(index);
                    }}
