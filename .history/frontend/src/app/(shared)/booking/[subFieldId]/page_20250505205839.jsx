...
export default function Booking() {
  ...
  function toggleSelectSlot(index) {
    ...
    if (start === "00:00" || end === "00:00") {
      alert("ไม่สามารถเลือกช่วงเวลา 00:00 ร่วมกับช่วงเวลาอื่นได้");
      return;
    }
    ...
    if (selectedSlots.length === 0) {
      ...
      const existingIndex = selectedSlots[0];
      if (existingIndex === index) {
        setSelectedSlots([]);
      } else {
        // ถ้ามี slot ใดเป็น 00:00 ห้ามรวม
        const selectedStart = slots[existingIndex].split(" - ")[0];
        const selectedEnd = slots[existingIndex].split(" - ")[1];
        if (selectedStart === "00:00" || selectedEnd === "00:00" || start === "00:00" || end === "00:00") {
          alert("ช่วงเวลา 00:00 ต้องเลือกแค่ช่วงเดียว");
          return;
        }
