...
export default function Booking() {
  ...
  const fetchBookedSlots = async () => {
    ...
    setDataLoading(true);
    try {
      // แปลงวันที่เป็น ISO format YYYY-MM-DD
      const start = new Date(bookingDate).toISOString().split("T")[0];
  ...
  // socket.io connect ครั้งเดียวตอน mount
  useEffect(() => {
    ...
    socket.on("slot_booked", (data) => {
      console.log("Slot booked event received:", data.bookingId);
  ...
  // fetch initial booked slots และ fetch ใหม่เมื่อ bookingDate หรือ subFieldId เปลี่ยน
  useEffect(() => {
    fetchBookedSlots();
  }, [bookingDate, subFieldId]);

  // fetch ใหม่เมื่อมี event slot_booked (bookingId เปลี่ยน)
  useEffect(() => {
    if (bookingId) {
      fetchBookedSlots();
    }
