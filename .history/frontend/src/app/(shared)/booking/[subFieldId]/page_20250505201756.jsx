...
export default function Booking() {
  ...
  function isSlotBooked(slot) {
    ...
    // Handle the case where the slot crosses midnight
    if (slotEnd <= slotStart) {
      slotEnd.setDate(slotEnd.getDate() + 1); // Move the end time to the next day
    }
  
    // Loop through booked slots and check if the current slot overlaps any of them
    for (let booking of bookedSlots) {
      ...
      // Handle bookings that also span midnight
      if (bookingEnd <= bookingStart) {
        bookingEnd.setDate(bookingEnd.getDate() + 1); // Move booking end to the next day
      }
  
      // Check if the selected slot overlaps with any booking
