...
export default function MyHours() {
  ...
  useEffect(() => {
    if (!field_id) {
      alert("field_id is not defined. Skipping fetch.");
    ...
    const fetchData = async () => {
      try {
        ...
        if (!data.error && data.data) {
          ...
          setFacilities(fac);
          alert(fac); // แสดงข้อมูลที่ได้จาก API
        } else {
      ...
      try {
        ...
        if (!data.error) {
          ...
          if (subField) {
            ...
            alert("ไม่พบ subField ตาม subFieldId");
          }
        } else {
          alert("ไม่พบข้อมูลวันเปิดสนาม");
        }
      } catch (error) {
        router.push("/");
  ...
  const handlePriceOnChange = (e) => {
    ...
    if (selectedValue === "subFieldPrice") {
      ...
      alert("subField price:", price); // แสดงราคาที่เลือก
      ...
      alert("Available AddOns:", addOns);

      if (selectedAddOn) {
        setNewPrice(selectedAddOn.price); // อัปเดตราคา
        alert("Add-On price:", selectedAddOn.price);
        setActivity(selectedAddOn.content);
      } else {
        alert("Add-On not found for selected value:", selectedValue);
      }
    }
  };

  const handleCheckBox = (facId, facPrice, facName) => {
    alert("Selected Facilities before:", selectedFacilities);
  ...
  const handleSubmit = async () => {
    ...
    alert("Booking Data being sent:", bookingData);
  ...
  const showPrice = () => {
    // แสดงราคาใน UI
    alert(price);
    alert(bookingDateFormatted);
    alert(`new${newPrice}`);
    alert(`field_id${field_id}`);
    alert(facilities);
    alert(sumFac);
    alert(`มัดจำ${priceDeposit}`);
    alert(`${payMethod}`);
    alert(`${bookingDate}`);
    alert(`${activity}`);
    alert(`selectedFacilities${selectedFacilities}`);
    alert(JSON.stringify(selectedFacilities, null, 2));
    const facilityList = Object.values(selectedFacilities).map((item) => ({
        ...
        <div>
          {facilities.map((fac) => (
            <div key={fac.facility_id}>
              <input
                type="checkbox"
                checked={selectedFacilities[fac.facility_id] != undefined}
                onChange={() =>
                  handleCheckBox(fac.facility_id, fac.fac_price, fac.fac_name)
                }
              />
              <label>
