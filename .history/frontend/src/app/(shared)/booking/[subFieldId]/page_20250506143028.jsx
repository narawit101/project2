...
export default function Booking() {
  ...
  function isSlotBooked(slot) {
    const getSlotStatus = (slot) => {
      const match = bookedSlots.find((b) => b.start_time === slot);
      return match ? match.status : null; // จะคืน 'approved', 'pending' หรือ null
    };
  ...
  {slots.map((slot, index) => {
    ...
    return (
      <div
        key={index}
        className={`slot-box ${slotClass}`}
        onClick={() => {
          if (!status) toggleSelectSlot(index);
        }}
