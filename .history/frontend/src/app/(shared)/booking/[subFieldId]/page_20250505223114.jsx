...
export default function Booking() {
  ...
  useEffect(() => {
    ...
    if (isBooked) {
      console.log("Triggering fetchBookedSlots..."); // เพิ่ม log
      fetchBookedSlots();
      setIsBooked(false); // รีเซ็ตสถานะ isBooked
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        alert(
          `Error: ${errorData.message || "Unknown error"} - Status Code: ${
            response.status
          }`
        );
      } else {
        ...
        if (data.success) {
          setBookingDate(null); // รีเซ็ต bookingDate
          setSubFieldId(null); // รีเซ็ต subFieldId
  
          // ทำให้ useEffect ดึงข้อมูลใหม่
          setBookingDate(bookingDateFormatted); // ตั้งค่ากลับเพื่อให้ useEffect ทำงานใหม่
          setSubFieldId(subFieldId); // ตั้งค่ากลับเพื่อให้ useEffect ทำงานใหม่
