...
export default function Booking() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const startDateObj = new Date(bookingDateFormatted);
      const endDateObj = new Date(bookingDateFormatted);
      endDateObj.setDate(endDateObj.getDate() + 1);

      const start = startDateObj.toISOString().split("T")[0];
      const end = endDateObj.toISOString().split("T")[0];

      console.log(`Fetching booked slots from ${start} to ${end}`);
  ...
  }, [bookingDate, subFieldId, API_URL]);

  // useEffect สำหรับ socket connect และ listen event slot_booked
  useEffect(() => {
    ...
    socket.on("slot_booked", (data) => {
      console.log("slot_booked event received:", data.bookingId);
      fetchBookedSlots(); // โหลดข้อมูลใหม่ทันทีเมื่อมี event
    });
  ...
  }, [API_URL, fetchBookedSlots]);

  // useEffect สำหรับ fetch เมื่อ bookingDate หรือ subFieldId เปลี่ยน
  useEffect(() => {
    fetchBookedSlots();
  }, [fetchBookedSlots]);

  // สมมติคุณมี state isBooked เพื่อ trigger fetch เพิ่มเติม
  useEffect(() => {
    if (isBooked) {
      fetchBookedSlots();
      setIsBooked(false);
