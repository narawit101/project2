...
export default function Booking() {
  ...
  const getSlotStatus = (slot) => {
    const booking = bookedSlots.find((b) => b.start_time === slot);
  ...
  {slots.map((slot, index) => {
    ...
    const isSelected =
      selectedSlots.length > 0 && index >= minIndex && index <= maxIndex;

    const status = getSlotStatus(slot); // <-- ใช้สถานะจริง
    ...
    return (
      <div
        ...
        className={`slot-box 
          ...
          ${isPending ? "pending-slot" : ""} 
          ${isApproved ? "approved-slot" : ""}`}
        onClick={() => {
