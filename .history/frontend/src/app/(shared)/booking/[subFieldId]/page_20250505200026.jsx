...
export default function Booking() {
  ...
  function isSlotBooked(slot) {
    ...
    const slotStartDate = new Date(`${bookingDate}T${start}`);
    const slotEndDate = new Date(`${bookingDate}T${end}`);
  
    // กรณีข้ามวัน (end น้อยกว่า start)
    if (slotEndDate <= slotStartDate) {
      slotEndDate.setDate(slotEndDate.getDate() + 1);
    }
  
    for (let booking of bookedSlots) {
      const bookingStartDate = new Date(`${bookingDate}T${booking.start_time}`);
      const bookingEndDate = new Date(`${bookingDate}T${booking.end_time}`);
      if (bookingEndDate <= bookingStartDate) {
        bookingEndDate.setDate(bookingEndDate.getDate() + 1);
      }
  
      // เปรียบเทียบช่วงเวลาแบบจริงจัง
      if (
        slotStartDate < bookingEndDate &&
        slotEndDate > bookingStartDate
      ) {
        return true;
      }
    }
