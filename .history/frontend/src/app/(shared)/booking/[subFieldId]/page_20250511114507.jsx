...
export default function Booking() {
  ...
  useEffect(() => {
    ...
    const fetchBookedSlots = async () => {
      try {
        ...
        dayAfterTomorrow.setDate(today.getDate() + 5); // ดึงถึงวันหลังจากวันถัดไป
  ...
  function getSlotStatus(slot) {
    ...
    for (let booking of bookedSlots) {
      const [bStartHour, bStartMin] = booking.start_time
        .slice(0, 5)
        .split(":")
        .map(Number);
      const [bEndHour, bEndMin] = booking.end_time
        .slice(0, 5)
        .split(":")
        .map(Number);
      ...
      for (let shift of shifts) {
        ...
        if (
          slotStartMin < shiftedBookingEnd &&
          slotEndMin > shiftedBookingStart
        ) {
