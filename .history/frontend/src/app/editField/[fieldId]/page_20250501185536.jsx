  ...
  const handleDeleteFacility = async (fieldFacId) => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/field/facilities/${fieldId}/${fieldFacId}`,
        {
          method: "DELETE",
          credentials: "include",
        }
      ...
      if (!res.ok) {
        console.error('Error response:', await res.json());  // log error details
      }
      if (res.ok) {
        // ลบสิ่งอำนวยความสะดวกที่ถูกลบออกจาก state
        setFacilities((prev) =>
          prev.filter((facility) => facility.field_fac_id !== fieldFacId)
        );
              ...
              <div className="facbox">
                {facilities.map((facility, index) => (
                  <div className="facitem" key={`${facility.fac_id}-${index}`}>
                    ...
                    <button
                      onClick={() =>
                        handleDeleteFacility(facility.field_fac_id)
                      }              
                    >
                      ลบ
                    </button>
