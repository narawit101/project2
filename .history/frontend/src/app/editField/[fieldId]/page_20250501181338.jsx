...
export default function CheckFieldDetail() {
  ...
  const handleSaveFacilities = async () => {
    ...
    for (const [facilityData] of Object.entries(selectedFacilities)) {
      if (!facilityData.fac_price || facilityData.fac_price < 0) {
    ...
    try {
      ...
      if (res.ok) {
         // เพิ่ม facilities ใหม่ทันทีที่บันทึกสำเร็จ
      setFacilities((prevFacilities) => [
        ...prevFacilities,
        ...Object.values(selectedFacilities), // เพิ่มข้อมูลที่ส่งไปใน POST
      ]);
        setMessage("เพิ่มสิ่งอำนวยความสะดวกสำเร็จ");
        setMessageType("success");
        // เรียกฟังก์ชันเพื่อดึงข้อมูลใหม่
      } else {
