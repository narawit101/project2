...
export default function CheckFieldDetail() {
  ...
  const [newFacilityName, setNewFacilityName] = useState("");
   ...
   // สำหรับเก็บชื่อใหม่ของสิ่งอำนวยความสะดวก
    ...
    const res = await fetch(
      `${API_URL}/facilities/update/${editingFacility.fac_id}`,
      {
        ...
        body: JSON.stringify({
          ...
          fac_price: editingFacility.fac_price, // ส่งราคาด้วย
        }),
      }
    );
    ...
    <>
      ...
      <div className="container">
        ...
        <div className="field-details">
          ...
          {editingFacility?.fac_id === facility.fac_id ? (
  <>
    <input
      type="text"
    ...
    <input
      type="number"
      value={facility.fac_price}
      onChange={(e) => {
        const updated = facilities.map((f) =>
          f.fac_id === facility.fac_id
            ? { ...f, fac_price: Math.abs(e.target.value) }
            : f
        );
  ...
  <>
    ...
    <button
      ...
      onClick={() => {
        ...
        setNewFacilityName(facility.fac_name);
      }}
    >
      แก้ไข
    </button>
