...
export default function CheckFieldDetail() {
  ...
  const handleDeleteFacility = async (fieldFacId) => {
    const confirmDelete = window.confirm("คุณแน่ใจหรือไม่ว่าต้องการลบ?");
    
    if (!confirmDelete) return;
  
    try {
      const res = await fetch(`${API_URL}/field/facilities/${fieldId}/${fieldFacId}`, {
        method: "DELETE",
        credentials: "include",
      });
      ...
      if (res.ok) {
        // ลบสิ่งอำนวยความสะดวกที่ถูกลบออกจาก state
        setFacilities((prev) => prev.filter((facility) => facility.field_fac_id !== fieldFacId));
    ...
    <div className="facbox">
      {facilities.map((facility, index) => (
        <div className="facitem" key={`${facility.fac_id}-${index}`}>
          ...
          <button
            onClick={() => handleDeleteFacility(facility.field_fac_id)} // ส่ง field_fac_id แทน fac_id
          ...
          <button className="savebtn" onClick={handleSaveFacilities}>
            เพิ่มสิ่งอำนวยความสะดวกไปยังสนาม
          </button>
