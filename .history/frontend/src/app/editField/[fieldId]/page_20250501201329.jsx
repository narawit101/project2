...
export default function CheckFieldDetail() {
  ...
  const [facilityToDelete, setFacilityToDelete] = useState(null); // Track the facility to delete
  ...
  const handleDeleteFacility = async (field_id, field_fac_id) => {
    try {
  ...
  const handleDeleteClick = (facility) => {
    setFacilityToDelete(facility); // Set the facility to delete
  ...
  const handleConfirmDelete = () => {
    if (facilityToDelete) {
      handleDeleteFacility(fieldId, facilityToDelete.field_fac_id);
    }
    setIsModalOpen(false); // Close the modal after confirming
    setFacilityToDelete(null); // Clear selected facility
  };
    ...
    return (
      <div className="modal-overlay">
        <div className="modal-content">
          <h3>คุณต้องการลบสิ่งอำนวยความสะดวกนี้หรือไม่?</h3>
                ...
                {facilities.map((facility) => (
                  <div className="facitem" key={facility.field_fac_id}>
                    ...
                    <button
                      ...
                      onClick={() =>
                        handleDeleteFacility(fieldId, facility.field_fac_id)
                      }
                    >
                      ลบ
                    </button>
