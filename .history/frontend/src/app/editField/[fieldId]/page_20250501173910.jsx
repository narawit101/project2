...
export default function CheckFieldDetail() {
  ...
  const handleDeleteFacility = async (fieldFacilityId) => {
    const confirm = window.confirm("ยืนยันการลบ?");
  
    if (!confirm) return;
  
    const res = await fetch(`${API_URL}/field/facilities/item/${fieldFacilityId}`, {
      method: "DELETE",
      credentials: "include",
    });
    ...
    if (res.ok) {
      setFacilities((prev) =>
        prev.filter((fac) => fac.field_facility_id !== fieldFacilityId)
      );
      setMessage("ลบสำเร็จ");
      setMessageType("success");
    } else {
      setMessage(data.message || "ลบไม่สำเร็จ");
      setMessageType("error");
    }
  };
  ...
  <div key={`${facility.fac_id}-${facility.fac_price}`}>
    <strong>{facility.fac_name}</strong>: {facility.fac_price} บาท
    <button
      onClick={() => handleDeleteFacility(facility.fac_id, facility.fac_price)}
