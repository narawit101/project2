...
export default function CheckFieldDetail() {
  ...
  const upDateStatus = async () => {
    try {
      ...
      const res = await fetch(`${API_URL}/field/update-status/${field.field_id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
        ...
        body: JSON.stringify({
          status: "ผ่านการอนุมัติ", // ปรับค่า status ที่ต้องการอัปเดต
        }),
      });
      if (res.ok) {
        const updatedField = await res.json();
        console.log(updatedField);
        // คุณอาจจะต้องอัปเดตสถานะใน state ของคุณหลังจากอัปเดตสถานะแล้ว
      } else {
