...
export default function CheckFieldDetail() {
  ...
  const handleSaveFacilities = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/field/facilities/${fieldId}`, {
        ...
        credentials: "include",
        body: JSON.stringify({ selectedFacilities }), // ส่ง object
      });
  ...
  const handleDeleteFacility = async (fieldId, facilityId) => {
    const confirmDelete = window.confirm("คุณแน่ใจหรือไม่ว่าต้องการลบ?");
  
    if (!confirmDelete) return;
  
    try {
      const res = await fetch(`${API_URL}/field/facilities/${fieldId}/${facilityId}`, {
        method: "DELETE",
        credentials: "include",
      });
      ...
      if (res.ok) {
        // ตรวจสอบให้มั่นใจว่ากรองข้อมูลถูกต้อง
        setFacilities((prev) => prev.filter((f) => f.fac_id !== facilityId || f.field_id !== fieldId));
        setMessage(result.message || "ลบสิ่งอำนวยความสะดวกสำเร็จ");
        setMessageType("success");
      } else {
        setMessage(result.message || "เกิดข้อผิดพลาดในการลบสิ่งอำนวยความสะดวก");
        setMessageType("error");
      }
    } catch (err) {
      setMessage("ไม่สามารถเชื่อมต่อกับเซิร์ฟเวอร์ได้");
      ...
      <div className="container">
        ...
        <div className="field-details">
          ...
          <div className="field-facilities">
            <h1>สิ่งอำนวยความสะดวกสนาม</h1>
            {facilities.length === 0 ? (
              <p>ยังไม่มีสิ่งอำนวยความสะดวกสำหรับสนามนี้</p>
            ) : (
              <div className="facbox">
                {facilities.map((facility, index) => (
                  <div className="facitem" key={`${facility.fac_id}-${index}`}>
                    {" "}
                    ...
                    <button
                      ...
                      onClick={() =>
                        handleDeleteFacility(fieldId, facility.fac_id)
                      }
                    >
                      ลบ
                    </button>
          ...
          <div className="factcon">
            ...
            <button className="savebtn" onClick={handleSaveFacilities}>
