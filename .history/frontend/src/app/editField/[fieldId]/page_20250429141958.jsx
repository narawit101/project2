...
export default function CheckFieldDetail() {
  ...
  const addSubField = async (userId) => {
    ...
    if (!newSportId) {
      ...
      setMessageType("error");
      return;  // หยุดการทำงานหากไม่มีการเลือกประเภทกีฬา
    }
  
    try {
      // ส่งคำขอไปยัง API เพื่อเพิ่มสนามย่อย
      const response = await fetch(`${API_URL}/field/subfield/${fieldId}`, {
        ...
        body: JSON.stringify({
          ...
          user_id: userId,
          sport_id: newSportId, // ส่ง sport_id ไปด้วย
        }),
      });
  
      // หากคำขอไม่สำเร็จ ให้จัดการข้อผิดพลาด
      if (!response.ok) {
        const errorData = await response.json();
        console.error("API Error: ", errorData); // ตรวจสอบข้อผิดพลาดจาก API
        setMessage(errorData.message || "ไม่สามารถเพิ่มสนามย่อยได้");
        setMessageType("error");
        return;  // หยุดทำการทำงานต่อ
      }
  
      // รับข้อมูลของสนามย่อยใหม่
      const newField = await response.json();
  
      // หากเพิ่มสนามย่อยสำเร็จ
      setSubFields([...subFields, newField]);
      setMessage("เพิ่มสนามย่อยสำเร็จ");
      setMessageType("success");
  
      // รีเฟรชหน้าจอหลังจากเพิ่มสนามย่อย
      setTimeout(() => {
        location.reload();
      }, 1000);
  
    } catch (error) {
      // จัดการข้อผิดพลาดจากการเชื่อมต่อหรือคำขอ
