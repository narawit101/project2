...
import "@/app/editField/[fieldId]/EditField.module.css"
...
export default function CheckFieldDetail() {
  ...
  const upDateStatus = async () => {
    try {
      // ตรวจสอบว่า field.field_id และ token ถูกต้อง
      console.log("Field ID ที่ส่งไป:", field.field_id);
      console.log("Token:", localStorage.getItem("token")); // ตรวจสอบ token ว่าถูกต้อง

      const token = localStorage.getItem("token");

      const res = await fetch(
        `${API_URL}/field/update-status/${field.field_id}`, // ตรวจสอบ URL ให้ถูกต้อง
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
          ...
          body: JSON.stringify({
            status: "รอตรวจสอบ", // สถานะที่ต้องการอัปเดต
          }),
        }
      );

      // ตรวจสอบว่า API ส่งกลับคำตอบถูกต้อง
      if (res.ok) {
        setMessage("ส่งคำขอสำเร็จ");
        setMessageType("success");
        const updatedField = await res.json();
        console.log("Field ที่อัปเดตแล้ว:", updatedField);
        setTimeout(() => {
          router.push("/myfield");
        }, 2000);
      } else {
