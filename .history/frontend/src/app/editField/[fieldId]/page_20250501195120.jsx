...
export default function CheckFieldDetail() {
  ...
  const handleDeleteFacility = async (field_id, field_fac_id) => {
    console.log("Deleting field_id:", fieldId);
    console.log("Deleting field_fac_id:", facility.field_fac_id);

  
    try {
      const res = await fetch(`${API_URL}/field/facilities/${field_id}/${field_fac_id}`, {
        method: "DELETE",
        credentials: "include",
      });
      ...
      if (res.ok) {
        setFacilities((prev) => prev.filter((f) => f.fac_id !== field_fac_id));
  ...
  useEffect(() => {
    const fetchFacilities = async () => {
      try {
        ...
        const data = await response.json();
        setFacilities(data);
        console.log("Fetched facilities:", data); // 👈 สำคัญ! ตรวจว่า field_fac_id มาไหม
      } catch (err) {
        console.error(err);
      }
    };
