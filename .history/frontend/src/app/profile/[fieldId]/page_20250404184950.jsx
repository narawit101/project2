...
// login เลื่อนรูป
  ...
  const handlePrev = (postId, length) => {
    setImageIndexes((prev) => ({
      ...prev,
      [postId]: prev[postId] > 0 ? prev[postId] - 1 : length - 1,
    }));
  };
  ...
  const handleNext = (postId, length) => {
    setImageIndexes((prev) => ({
      ...prev,
      [postId]: prev[postId] < length - 1 ? prev[postId] + 1 : 0,
    }));
  };
          ...
          {postData.map((post, index) => (
            <div key={`${post.post_id}-${index}`} className="post-card">
              ...
              {post.images && post.images.length > 0 ? (
               ...
               <div className="image-box-smooth">
                 {post.images.map((img, idx) => (
                   <img
                     key={idx}
                     src={`${API_URL}/${img.image_url}`}
                     alt={`รูป ${idx + 1}`}
                     className={`carousel-img-smooth ${
                       imageIndexes[post.post_id] === idx ? "active" : ""
                     }`}
                   />
                 ))}
               </div>
               ...
               <div className="dot-indicators">
                 {post.images.map((_, dotIdx) => (
                   <span
                     key={dotIdx}
                     className={`dot ${
                       (imageIndexes[post.post_id] || 0) === dotIdx ? "active" : ""
                     }`}
                     onClick={() =>
                       setImageIndexes((prev) => ({
                         ...prev,
