...
export default function CheckFieldDetail() {
  ...
  }, [postData]); // ต้องรอให้ postData โหลดเสร็จก่อน
  // login เลื่อนรูป
  ...
  const handleCardClick = (newId,newIdSub) => {
    const fieldId = fieldData.field_id(newId);
    const subFieldId = fieldData.sub_fields[0].sub_field_id(newIdSub);

    // เก็บข้อมูล field_id และ sub_field_id ใน localStorage พร้อมกับเวลาเมื่อข้อมูลจะหมดอายุ
    const expiryDate = new Date();
    expiryDate.setMinutes(expiryDate.getMinutes() + 10); // ตั้งวันหมดอายุหลังจาก 10 นาที
    localStorage.setItem("field_id", fieldId);
    localStorage.setItem("sub_field_id", subFieldId);
    localStorage.setItem("id_expiry_date", expiryDate.toString());

    // นำทางไปที่หน้า Calendar
    router.push(`/calendar/${subFieldId}`);
  };
          ...
          <div className="sub-fields-container">
            ...
            {fieldData?.sub_fields && fieldData.sub_fields.length > 0 ? (
              fieldData.sub_fields.map((sub) => (
                <div key={sub.sub_field_id} className="sub-field-card" onClick={handleCardClick}>
                  <p>
