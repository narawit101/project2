...
export default function CheckFieldDetail() {
  ...
  const handleEditSubmit = async (e, postId) => {
   ...
   for (const img of newImages) {
    if (!img.type.startsWith("image/")) {
      alert(`ไฟล์ ${img.name} ไม่ใช่รูปภาพ`);
      return;
    }
    if (img.size > 8 * 1024 * 1024) {
      alert(`ไฟล์ ${img.name} มีขนาดเกิน 8MB`);
      return;
    }
  }
    // ตรวจสอบห้ามเกิน 10 รูป
    if (existingImageCount + newImageCount > 10) {
      alert("รวมรูปทั้งหมดต้องไม่เกิน 10 รูป (รวมรูปเดิมและรูปใหม่)");
      return;
    }
  ...
  const handleDelete = async (postId) => {
    if (!confirm("คุณแน่ใจหรือไม่ว่าต้องการลบโพสนี้?")) return;
    ...
    try {
      ...
      if (res.ok) {
        ...
        setPostData((prev) => prev.filter((post) => post.post_id !== postId));
      } else {
      ...
      <div className="field-detail-container">
        ...
        <div key={post.post_id} className="post-card">
          {editingPostId === post.post_id ? (
            <form onSubmit={(e) => handleEditSubmit(e, post.post_id)} className="edit-form">
            ...
            <>
                ...
                <div className="ig-carousel-container">
                  <div className="ig-carousel-track-wrapper">
                    <div
                      ...
                      style={{
                        transform: `translateX(-${(imageIndexes[post.post_id] || 0) * 100}%)`,
                      }}
                    >
                      {post.images.map((img, idx) => (
                        <img
                          ...
                          onClick={() => setSelectedImage(`${API_URL}/${img.image_url}`)}
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        setImageIndexes((prev) => ({
                          ...prev,
                          [post.post_id]: (prev[post.post_id] || 0) - 1 < 0 ? len - 1 : (prev[post.post_id] || 0) - 1,
                        }));
                      }}
                    >
                      ❮
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        setImageIndexes((prev) => ({
                          ...prev,
                          [post.post_id]: (prev[post.post_id] || 0) + 1 >= len ? 0 : (prev[post.post_id] || 0) + 1,
                        }));
                      }}
                    >
                      ❯
                  ...
                  <div className="dot-indicators">
                    {post.images.map((_, dotIdx) => (
                      <span
                        key={dotIdx}
                        className={`dot ${(imageIndexes[post.post_id] || 0) === dotIdx ? "active" : ""}`}
                        onClick={() =>
                          setImageIndexes((prev) => ({
                            ...prev,
                            [post.post_id]: dotIdx,
              ...
              {canPost && (
                <div className="post-actions">
                  <button onClick={() => handleEdit(post)} className="btn">แก้ไขโพส</button>
                  <button onClick={() => handleDelete(post.post_id)} className="btn">ลบโพส</button>
                </div>
              )}
            </>
          )}
