...
export default function CheckFieldDetail() {
  ...
  useEffect(() => {
    if (!fieldId) {
      ...
      return;  // If no fieldId is present, stop execution
    }
    ...
    fetch(`${API_URL}/posts/${fieldId}`, {
      ...
      .then((res) => {
        if (!res.ok) {
          throw new Error("Failed to fetch posts"); // If response status is not OK, throw an error
        }
        return res.json();
      })
      .then((data) => {
        // Check if the data contains posts and handle the case when there are no posts
        if (data.error) {
          ...
          if (data.posts && data.posts.length > 0) {
            ...
            setPostData(data.posts); // Update state with post data
          } else {
            console.log("No posts available.");
            setPostData([]); // Set an empty array if no posts are found
          }
        }
      })
      .catch((error) => {
        console.error("Error fetching post data:", error);  // Log any errors
      });
  }, [fieldId, router]);
      ...
      <div className="field-detail-container">
        <aside>
          ...
          <div className="content">
            ...
            <div className="post">
  <h1>โพสทั้งหมด</h1>
  <Post fieldId={fieldId}></Post>

  {/* Check if postData is an array and has elements */}
  {Array.isArray(postData) && postData.length > 0 ? (
    postData.map((post) => (
      <div key={post.post_id} className="post-card">
        ...
        {post.image_url ? (
          ...
          />
        ) : null} {/* If there's no image, render nothing */}
    ...
    <p className="custom-no-fields-message">ยังไม่มีโพส</p> // Display this message if no posts
  )}
</div>
