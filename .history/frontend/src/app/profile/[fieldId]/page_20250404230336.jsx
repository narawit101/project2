...
export default function CheckFieldDetail() {
  ...
  const handleImageChange = (e) => {
    ...
    for (const file of files) {
      if (!file.type.startsWith("image/")) {
        setMessage(`ไฟล์ ${file.name} ไม่ใช่รูปภาพ`);
        setMessageType("error");
        e.target.value = null;
        return;
      }
      if (file.size > 8 * 1024 * 1024) {
        setMessage(`ไฟล์ ${file.name} มีขนาดเกิน 8MB`);
        setMessageType("error");
        e.target.value = null;
        return;
      }
  ...
  const handleEditSubmit = async (e, postId) => {
    ...
    if (res.ok) {
      // สร้าง array ของรูปใหม่เพื่อเพิ่มเข้า post.images
      const newImageUrls = newImages.map((file) => ({
        image_url: `uploads/images/posts/${file.name}`, // เปลี่ยนตรงนี้ตาม path backend เก็บรูป
      }));
    
      // อัปเดต postData ใน frontend
      setPostData((prev) =>
        prev.map((post) =>
          post.post_id === postId
            ? {
      ...
      setNewImages([]);
      setMessage("แก้ไขโพสสำเร็จ");
      setMessageType("success");
    } else {
      setMessage("แก้ไขโพสไม่สำเร็จ");
      setMessageType("error");
    }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setMessage("เกิดข้อผิดพลาดในการลบโพส");
        setMessageType("error");
      }
    } catch (error) {
      console.error("Delete error:", error);
      setMessage("ลบโพสไม่สำเร็จ");
      setMessageType("error");
    }
  };
    ...
    <>
      ...
      {showModal && (
        <div className="modal-overlay">
          <div className="modal">
            <p>คุณแน่ใจหรือไม่ว่าต้องการลบโพสนี้?</p>
