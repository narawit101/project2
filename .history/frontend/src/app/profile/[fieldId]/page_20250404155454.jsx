...
export default function CheckFieldDetail() {
  ...
  useEffect(() => {
    // Fetch the facilities from the backend when the component mounts
  ...
  return (
    <>
      ...
      <h2>สิ่งอำนวยความสะดวกสำหรับสนามนี้</h2>
      {facilities.length === 0 ? (
        <p>ยังไม่มีสิ่งอำนวยความสะดวกสำหรับสนามนี้</p> // No facilities message in Thai
      ) : (
        <ul>
              ...
              {Array.isArray(postData) && postData.length > 0 ? (
                postData.map((post, index) => (
                  <div key={`${post.post_id}-${index}`} className="post-card">
                    ...
                    {/* Use a combination of post_id and index */}
                    {post.image_url ? (
                      <img
