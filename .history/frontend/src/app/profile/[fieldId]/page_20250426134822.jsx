...
export default function CheckFieldDetail() {
  ...
  const handleCardClick = (newId, newIdSub) => {
    ...
    // เก็บข้อมูล field_id และ sub_field_id ใน localStorage
    localStorage.setItem("field_id", newId);  // ใช้ newId โดยตรง
    localStorage.setItem("sub_field_id", newIdSub);  // ใช้ newIdSub โดยตรง
    localStorage.setItem("id_expiry_date", expiryDate.toString());

    // นำทางไปที่หน้า Calendar
    router.push(`/calendar/${newIdSub}`);
  };
