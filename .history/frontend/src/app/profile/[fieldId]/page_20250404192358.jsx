...
export default function CheckFieldDetail() {
  ...
  const handleLightboxPrev = () => {
    ...
    const images = currentPost?.images?.filter((img) => img?.image_url) || [];
    if (images.length === 0) return;
    const currentIndex = images.findIndex((img) => `${API_URL}/${img.image_url}` === selectedImage);
  ...
  return (
    ...
    {selectedImage && (
        <div className="lightbox-overlay" onClick={handleCloseLightbox}>
          <button onClick={(e) => { e.stopPropagation(); handleLightboxPrev(); }} className="lightbox-arrow left" style={{ fontSize: '30px', padding: '10px', backgroundColor: 'rgba(0,0,0,0.4)', color: 'white', border: 'none', borderRadius: '50%' }}>❮</button>
          <img src={selectedImage} alt="Zoomed" className="lightbox-image" />
          <button onClick={(e) => { e.stopPropagation(); handleLightboxNext(); }} className="lightbox-arrow right" style={{ fontSize: '30px', padding: '10px', backgroundColor: 'rgba(0,0,0,0.4)', color: 'white', border: 'none', borderRadius: '50%' }}>❯</button>
        </div>
      )}
      {fieldData?.img_field ? (
