...
export default function CheckFieldDetail() {
  ...
  const handleNext = (postId, imagesLength) => {
    setImageIndexes((prev) => ({
      ...prev,
      [postId]: (prev[postId] + 1 || 1) % imagesLength,
    }));
  };
  ...
  const handleDragging = (e) => {
    if (!isDragging || dragPostId === null) return;
    ...
    if (Math.abs(diff) > 50) {
      if (diff > 0) {
        handlePrev(dragPostId);
      } else {
        handleNext(dragPostId);
      }
      setIsDragging(false);
      setDragPostId(null);
    }
        ...
        <div className="post">
          ...
          <div className="carousel-container-swipe">
            <div
              ...
              onMouseDown={(e) => handleDragStart(e, post.post_id)}
              onMouseMove={(e) => handleDragging(e)}
              onMouseUp={handleDragEnd}
              onMouseLeave={handleDragEnd}
              onTouchStart={(e) => handleDragStart(e, post.post_id)}
              onTouchMove={(e) => handleDragging(e)}
            ...
            >
              {post.images.map((img, idx) => (
                <img
                  ...
                  className="carousel-image-swipe"
                  alt={`img-${idx}`}
                />
              ))}
            </div>
