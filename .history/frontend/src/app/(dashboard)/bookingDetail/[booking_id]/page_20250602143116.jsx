...
export default function BookingDetail() {
  ...
  <h1 className="order-detail-title">รายการจองของคุณ</h1>
  ...
  {!booking || Object.keys(booking).length === 0 ? (
    ...
    <li className="booking-card-order-detail">
      <p><strong>ชื่อผู้จอง:</strong> {booking.first_name} {booking.last_name}</p>
      <p><strong>วันที่:</strong> {formatDate(booking.start_date)}</p>
      <p><strong>เวลา:</strong> {booking.start_time} - {booking.end_time}</p>
      <p><strong>ชั่วโมงรวม:</strong> {calTotalHours(booking.total_hours)} น.</p>
      <p><strong>ยกเลิกได้ถึงเวลา:</strong> {getCancelDeadlineTime(booking.start_date, booking.start_time, booking.cancel_hours)} น.</p>
      <p><strong>สนาม:</strong> {booking.field_name}</p>
      <p><strong>สนามย่อย:</strong> {booking.sub_field_name}</p>
      <p><strong>กิจกรรม:</strong> {booking.activity}</p>
      <p><strong>มัดจำ:</strong> {booking.price_deposit}</p>
      <div><img src={`${API_URL}/${booking.deposit_slip}`} alt="สลิป" /></div>
      <p><strong>ยอดค้างชำระ:</strong> {booking.total_remaining}</p>

      {(user?.user_id === booking.field_user_id || user?.role === "admin") && (
        <div className="status-buttons-order-detail">
          <button className="approve-btn-order-detail" onClick={() => openConfirmModal("approved")}>อนุมัติ</button>
          <button className="reject-btn-order-detail" onClick={() => openConfirmModal("rejected")}>ไม่อนุมัติ</button>
        </div>
      )}

      <button className="cancel-booking-btn-order-detail" onClick={() => cancelBooking(booking.booking_id)}>
        ยกเลิกการจอง
      </button>
    </li>
  )}
