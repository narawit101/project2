...
export default function MyCalendar() {
        ...
        .then((data) => {
          if (data.error) {
            ...
            if (data[0] && data[0].open_days) {
              ...
              const selectedSubField = data[0].sub_fields.find(
  ...
  const tileClassName = ({ date, view }) => {
    const day = date.getDay();
    if (view === "month" && opendays.includes(day) && date <= maxDate && date >= today) {
  ...
  return (
    <div>
      ...
      <div className="select-day">
        <p>เลือกวันที่: {formatDateToThai(date)}</p>
      ...
      <div className="save-btn">
        <button onClick={handleDateConfirm}>เลื่อช่วงเวลา</button>
      </div>
    </div>
  );
}
