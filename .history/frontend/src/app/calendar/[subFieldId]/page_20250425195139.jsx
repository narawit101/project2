...
export default function MyCalendar() {
  ...
  const [isClient, setIsClient] = useState(false); // Flag to check if we are on the client
  ...
  if (!isClient) return <div>Loading...</div>; // Prevent hydration error by rendering on the client only
  return (
    <div>
      ...
      <button onClick={() => router.push("/")}>ยืนยัน</button>
      </div>
    </div>
  );
}
