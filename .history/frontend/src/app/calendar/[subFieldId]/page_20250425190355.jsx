...
export default function MyCalendar() {
  ...
  const [date, setDate] = useState(null); // ตั้งค่าเริ่มต้นเป็น null (ไม่แสดงวันที่ปัจจุบัน)
  const router = useRouter();
  const [opendays, setOenDays] = useState([]);
  const { subFieldId } = useParams();

  // ตรวจสอบว่า `localStorage` มีวันที่เก็บไว้อยู่และเช็ควันหมดอายุ
  useEffect(() => {
    const storedData = localStorage.getItem("booking_date"); // ตรวจสอบว่ามีวันที่เก็บใน localStorage หรือไม่
    const storedDate = localStorage.getItem("booking_date_expiry");

    if (storedData && storedDate) {
      const expiryDate = new Date(storedDate);
      const currentDate = new Date();

      // ถ้าวันหมดอายุยังไม่ถึง ให้แสดงวันที่ที่เก็บไว้
      if (currentDate < expiryDate) {
        setDate(new Date(storedData));
      } else {
        localStorage.removeItem("booking_date");
        localStorage.removeItem("booking_date_expiry"); // ลบข้อมูลถ้าวันหมดอายุ
        setDate(null); // ตั้งค่าให้ไม่มีวันที่แสดง
      }
    }
  }, []);

  // เซ็ตข้อมูลวันเปิดสนาม
  useEffect(() => {
    setDate(null); // เริ่มต้นเป็น null
    const daysNumbers = {
      Sun: 0,
      Mon: 1,
      Tue: 2,
