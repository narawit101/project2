...
export default function MyCalendar() {
        ...
        .then((data) => {
          if (data.error) {
            ...
            if (data[0] && data[0].open_days) {
              ...
              const subFieldDetail =
                data[0].sub_fields[0] || "ไม่พบข้อมูล";
              setFieldData(subFieldDetail);
              console.log("ข้อมูลสนาม", data);
              console.log("ข้อมูลสนามย่อย", subFieldDetail);
  ...
  return (
    ...
    <h2>ข้อมูลสนามย่อย</h2>
    
    {/* ตรวจสอบว่า fieldData มีข้อมูลหรือไม่ */}
    {fieldData !== "ไม่พบข้อมูล" ? (
      <div>
        <p><strong>ชื่อสนามย่อย:</strong> {fieldData.sub_field_name}</p>
        <p><strong>ราคา:</strong> {fieldData.price} บาท</p>
        <p><strong>กีฬา:</strong> {fieldData.sport_name}</p>

        {/* แสดง Add-ons ถ้ามี */}
        {fieldData.add_ons && fieldData.add_ons.length > 0 ? (
          <div className="add-ons-container">
            <h3>ราคาสำหรับกิจกรรมพิเศษ</h3>
            {fieldData.add_ons.map((addon) => (
      ...
      <p>{fieldData}</p> {/* แสดงข้อความ 'ไม่พบข้อมูล' ถ้าไม่มีข้อมูล */}
    )}
  </div>
