        ...
        .then((data) => {
          if (data.error) {
            ...
            if (data[0] && data[0].open_days) {
              ...
              const selectedSubFieldId = 177; // ตัวอย่างการใช้ ID ที่ได้รับจาก URL หรือผู้ใช้เลือก

              // ค้นหาข้อมูลของสนามย่อยที่ตรงกับ sub_field_id ที่เลือก
              const selectedSubField = data[0].sub_fields.find(sub => sub.sub_field_id === selectedSubFieldId) || "ไม่พบข้อมูล";
        ...
        {fieldData !== "ไม่พบข้อมูล" ? (
          <div>
            <p className="sub-name">
              <strong>ชื่อสนาม</strong> {fieldData.sub_field_name}
            </p>
            <p className="price">
