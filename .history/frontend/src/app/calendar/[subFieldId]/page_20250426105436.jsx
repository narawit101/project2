...
export default function MyCalendar() {
  ...
  {fieldData?.sub_fields && fieldData.sub_fields.length > 0 ? (
    <div key={subField?.sub_field_id} className="sub-field-card" onClick={() => router.push(`/calendar/${subField?.sub_field_id}`)}>
      <p><strong>ชื่อสนาม:</strong> {subField?.sub_field_name}</p>
      <p><strong>ราคา:</strong> {subField?.price} บาท</p>
      <p><strong>กีฬา:</strong> {subField?.sport_name}</p>

      {/* แสดง Add-ons ถ้ามี */}
      {subField?.add_ons && subField.add_ons.length > 0 ? (
        <div className="add-ons-container">
          <h3>ราคาสำหรับจัดกิจกรรมพิเศษ</h3>
          {subField.add_ons.map((addon) => (
