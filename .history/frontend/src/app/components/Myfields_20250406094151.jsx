...
export default function MyFieldPage() {
  ...
  const [canEdit, setCanEdit] = useState(true); // State for checking if the user can post
    ...
    const fetchMyFields = async () => {
      try {
        ...
        if (data && data.length > 0) {
          ...
          setCanEdit(status !== "รอตรวจสอบ"); // จะตั้งค่า canEdit เป็น false ถ้า status คือ "รอตรวจสอบ"
        }
