...
export default function MyFieldPage() {
  ...
  const handleDeleteField = async(field_id)=>{
    const token = localStorage.getItem("token");
    try {
      const res = await fetch(`${API_URL}/field/delete/field/${field_id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
      ...
      setMyFields(myFields.filter((field) => field.field_id !== field_id));
      setFilteredFields(filteredFields.filter((field) => field.field_id !== field_id));
    } catch (error) {
