...
export default function Mybooking() {
  ...
  <div className="line-item minus">
    <span>มัดจำ</span>
    <span>- {item.price_deposit} บาท</span>
  </div>

  <div className="line-item plus">
    <span>สนาม</span>
    <span>+ {item.total_price - item.price_deposit - (item.facilities?.reduce((sum, f) => sum + f.fac_price, 0) || 0)} บาท</span>
  </div>
  ...
  {Array.isArray(item.facilities) && item.facilities.length > 0 && (
    <div className="line-item plus">
      ...
      <span>
        +{" "}
        {item.facilities.reduce((sum, f) => sum + f.fac_price, 0)} บาท
      </span>
    </div>
  )}
  ...
  <div className="line-item total">
    <span>สุทธิ</span>
    <span>{item.total_remaining} บาท</span>
  </div>
</div>
