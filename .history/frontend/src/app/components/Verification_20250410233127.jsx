...
export default function Verification() {
  ...
  if (storedUser) {
    const data = JSON.parse(storedUser); // ทำการแปลงข้อมูลเมื่อแน่ใจว่ามีค่า
  ...
  const noSave = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const newToken = result.token;  // หรือค่าจากการตอบกลับ API
        const newUser = result.user;  // หรือค่าจากการตอบกลับ API
        setTimeout(() => {
          localStorage.setItem("token", newToken); // ต้องระบุทั้งคีย์และค่า
          localStorage.setItem("user", JSON.stringify(newUser)); // ต้องระบุทั้งคีย์และค่า
        }, 1500);
      } else {
