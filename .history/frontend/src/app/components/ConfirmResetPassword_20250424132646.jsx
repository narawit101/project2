...
export default function ConfirmResetPassword() {
  ...
  const handlePasswordChange = async (e) => {
    ...
    try {
      // ตรวจสอบว่า token และ user_id มีใน localStorage หรือไม่
      if (!token || !user) {
      ...
      // ทำการส่งคำขอ PUT ไปที่ API เพื่อเปลี่ยนรหัสผ่าน
      const response = await fetch(
        `${API_URL}/users/${user.user_id}/change-password`,
        {
          ...
          body: JSON.stringify({
            password: newPassword, // ส่งรหัสผ่านใหม่ไปใน body
          }),
        }
      );
  ...
  return (
    <div>
      ...
      <div className="container">
        ...
        />
        <button type="submit" className="save-btn">ยืนยัน</button>
        </form>
      </div>
    </div>
  );
