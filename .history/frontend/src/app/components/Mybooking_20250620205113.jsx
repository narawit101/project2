...
export default function Mybooking() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_URL}/booking/my-bookings/${user.user_id}?${queryParams.toString()}`,
        { credentials: "include" }
      );
      ...
      if (data.success) {
        ...
        setUserInfo(`${data.user?.first_name || ""} ${data.user?.last_name || ""}`);
        setMessage("");
  ...
  // เรียก fetchData ตอน mount และเมื่อ filters หรือ user.user_id เปลี่ยน
  useEffect(() => {
    fetchData();
  }, [fetchData]);

  // ตั้ง socket connection และ listen event slot_booked
  useEffect(() => {
    ...
    socket.on("slot_booked", () => {
      ...
      fetchData(); // โหลดข้อมูลใหม่เมื่อมีคนจอง slot
    });
