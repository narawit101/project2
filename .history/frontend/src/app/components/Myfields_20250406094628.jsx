    ...
    const fetchMyFields = async () => {
      try {
        ...
        if (data && data.length > 0) {
          const status = data[0].status;  // สมมติว่า status อยู่ใน object แรกใน array
          console.log("Status:", status);  // เพิ่มบรรทัดนี้เพื่อตรวจสอบสถานะที่ได้จาก API
  
          // ตรวจสอบสถานะและตั้งค่า canEdit ตามสถานะ
          setCanEdit(status !== "รอตรวจสอบ");
        }
        console.log("canEdit:", canEdit);  // เพิ่มบรรทัดนี้เพื่อตรวจสอบค่าของ canEdit
