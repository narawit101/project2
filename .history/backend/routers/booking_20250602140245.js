...
router.get("/bookings-detail/:booking_id", authMiddleware, async (req, res) => {
  ...
  try {
    // ดึงข้อมูล booking พร้อม owner_id ของสนามด้วย
    const result = await pool.query(
      `
      SELECT 
        b.*,
        ...
        f.price_deposit,
        f.cancel_hours,
        f.owner_id AS field_owner_id, -- 👈 เพิ่ม owner_id
        sf.sub_field_name,
        sf.price,
        bf.fac_name AS facility_name,
        bf.field_fac_id,
    ...
    const isFieldOwner = requestingUser.user_id === booking.field_owner_id;
