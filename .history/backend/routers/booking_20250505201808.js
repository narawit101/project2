...
router.get("/booked-time/:subFieldId/:bookingDate", async (req, res) => {
  ...
  try {
    ...
    const nextBookingDate = nextDate.toISOString().split("T")[0];

    // Query both current and next day for bookings that may span across midnight
    const result = await client.query(
      `SELECT start_time, end_time, booking_date, status
       FROM bookings
       WHERE sub_field_id = $1
         AND booking_date IN ($2, $3)
         AND status IN ('pending', 'approved', 'rejected')`,
      [subFieldId, bookingDate, nextBookingDate]
    );
