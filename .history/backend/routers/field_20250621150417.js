...
const storage = new CloudinaryStorage({
  ...
  params: async (req, file) => {
    ...
    if (isImage) {
      ...
      // สำหรับเอกสาร - รวมนามสกุลใน public_id
      return {
        folder: folder,
        resource_type: "raw",
        public_id: `${uniqueId}.${fileExtension}`, // เพิ่มนามสกุล
      };
    }
  },
});
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
    const uploadedFiles = {
      images: req.files.img_field || [],
      documents: req.files.documents || []
    };
    
    const response = {
      images: uploadedFiles.images.map(file => ({
        url: file.path,
        publicId: file.filename,
        originalName: file.originalname
      })),
      documents: uploadedFiles.documents.map(file => ({
        url: file.path,
        publicId: file.filename,
        originalName: file.originalname
      }))
...
async function deleteCloudinaryFile(fileUrl) {
  try {
    ...
    let resourceType = isRawFile ? "raw" : "image";
    
    // สำหรับรูปภาพ ตัดนามสกุลออก
    if (!isRawFile && fullPath.includes('.')) {
      const lastDotIndex = fullPath.lastIndexOf('.');
      publicId = fullPath.substring(0, lastDotIndex);
    }
    
    console.log(`🎯 ลบ: ${publicId} (${resourceType})`);
    ...
    if (result.result === "ok") {
      console.log(`✅ ลบสำเร็จ: ${publicId}`);
    } else {
      console.warn(`⚠️ ลบไม่สำเร็จ: ${publicId}`, result);
    }
