...
router.delete("/facilities/:field_id/:facility_id", async (req, res) => {
  const { field_id, facility_id } = req.params;

  try {
    // ตรวจสอบว่าเงื่อนไขการลบถูกต้อง
    const result = await pool.query(
      "DELETE FROM field_facilities WHERE field_id = $1 AND facility_id = $2",
