...
const storage = new CloudinaryStorage({
  ...
  params: async (req, file) => {
    ...
    // สำหรับไฟล์รูปภาพให้กำหนด format
    if (isImage) {
      params.format = fileExtension;
    }
    // สำหรับไฟล์เอกสาร ไม่ต้องกำหนด format (ให้ Cloudinary จัดการเอง)
    // หรือใช้ raw resource_type สำหรับไฟล์ที่ไม่ใช่รูปภาพ
    else if (isDocument) {
      params.resource_type = "raw"; // สำหรับไฟล์เอกสาร
      // ไม่ต้องกำหนด format สำหรับ raw files
    }
...
async function deleteCloudinaryFile(fileUrl) {
  try {
    ...
    const isImageFile = fileUrl.includes('/image/upload/') || 
    ...
    if (isRawFile) {
      ...
      publicId = lastDotIndex > 0 ? fullPath.substring(0, lastDotIndex) : fullPath;
      resourceType = "image";
      console.log("🖼️ ไฟล์รูปภาพ:", publicId);
    }
    ...
    if (result.result === "ok") {
      ...
      console.warn(`⚠️ ไม่พบไฟล์: ${publicId} (${resourceType})`);
      ...
      if (retryResult.result === "ok") {
        ...
        console.warn(`⚠️ ลบไม่สำเร็จทั้งสองแบบ: ${publicId}`, retryResult);
      }
    } else {
      console.warn(`⚠️ ผลลัพธ์ไม่คาดคิด: ${publicId}`, result);
    }
  } catch (error) {
...
async function deleteMultipleCloudinaryFiles(fileUrls) {
  if (!fileUrls || fileUrls.length === 0) {
    console.log("ℹ️ ไม่มีไฟล์ที่ต้องลบ");
    return;
  }
  
  console.log(`🗂️ กำลังลบไฟล์ ${fileUrls.length} ไฟล์`);
