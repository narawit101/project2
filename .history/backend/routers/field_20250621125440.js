...
async function deleteCloudinaryFile(fileUrl) {
  try {
    ...
    const folder = matches[1]; // เช่น field-profile
    const filename = matches[2]; // เช่น 1750483997582
    const publicId = `${folder}/${filename}`;

    await cloudinary.uploader.destroy(publicId, {
      resource_type: "auto", // auto สำหรับทั้ง image/pdf/อื่นๆ
    });
...
router.delete("/facilities/:field_id/:field_fac_id", authMiddleware, async (req, res) => {
