...
router.put("/update-status/:field_id", authMiddleware, async (req, res) => {
  try {
    ...
    // ตรวจสอบว่า status ที่ได้รับมาเป็น "รออนุมัติ" หรือไม่
    if (status !== "รออนุมัติ") {
      return res.status(400).json({ error: "สถานะไม่ถูกต้อง" });
    }

    // ทำการอัปเดตสถานะของสนาม
    const result = await pool.query(
      `UPDATE field
       SET status = $1
       WHERE field_id = $2
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({ error: "ไม่พบสนามที่ต้องการอัปเดต" });
    }

    res.json({ message: "อัปเดตสถานะสำเร็จ", data: result.rows[0] });
  } catch (error) {
    console.error("Database Error:", error);
    res.status(500).json({ error: "เกิดข้อผิดพลาดในการอัปเดตสถานะ", details: error.message });
  }
});
