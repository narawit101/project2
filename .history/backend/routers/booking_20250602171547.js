...
router.delete(
  ...
  async (req, res) => {
    ...
    try {
     ...
     // ตรวจสอบว่าเป็น string ที่มีรูปแบบ HH:mm หรือ HH:mm:ss
if (
  !start_time ||
...
// ตัด start_time เหลือแค่ HH:mm (ปลอดภัยสุด)
const trimmedStartTime = start_time.slice(0, 5); // "19:00"
const startDateTimeRaw = `${startDateStr}T${trimmedStartTime}:00Z`;
