  ...
  try {
    ...
    for (const facId in selectedFacilities) {
      ...
      const user_id = req.
        ...
        const userData = await pool.query("SELECT * FROM users WHERE user_id = $1", [rows[0].user_id]);

        // สมมุติว่าในตาราง users มีคอลัมน์ชื่อ user_email
        const userEmail = userData.rows[0].user_email; // << ใช้ค่านี้ส่งอีเมล
...
router.put("/:field_id", authMiddleware, async (req, res) => {
  try {
   ...
   if (status === "ผ่านการอนุมัติ") {
    ...
    await pool.query(
      "UPDATE users SET role = 'field_owner' WHERE user_id = $1",  // แก้ไขเป็น "customer" ถ้าไม่ผ่านการอนุมัติ
      [userId]
    );
  }
    try {
