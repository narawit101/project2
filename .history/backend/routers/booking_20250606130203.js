...
router.get("/my-orders/:field_id", authMiddleware, async (req, res) => {
  ...
  try {
    // ตรวจสอบความเป็นเจ้าของสนาม (ถ้าไม่ใช่ admin)
    if (user_role !== "admin") {
    ...
    // กรองเฉพาะ booking ที่อนุมัติและไม่ซ้ำ booking_id
    const uniqueApprovedBookings = new Map();
    result.rows.forEach((row) => {
      if (row.status === "approved" && !uniqueApprovedBookings.has(row.booking_id)) {
        uniqueApprovedBookings.set(row.booking_id, row);
      }
    });

    const uniqueApproved = Array.from(uniqueApprovedBookings.values());
    ...
    const stats = {
      ...
      totalRevenue: uniqueApproved.reduce(
        (sum, row) => sum + parseFloat(row.total_price || 0),
        0
      ),
      totalDeposit: uniqueApproved.reduce(
        (sum, row) => sum + parseFloat(row.price_deposit || 0),
        0
      ),
    };
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to get bookings" });
  }
});


// เพิ่ม API สำหรับรายงานสรุป
router.get("/report/:field_id", authMiddleware, async (req, res) => {
  const { field_id } = req.params;
  ...
  try {
    ...
    if (user_role !== "admin") {
      ...
      if (fieldCheck.rows[0].user_id !== user_id) {
    ...
    switch (groupBy) {
      case 'month':
        dateGrouping = "DATE_TRUNC('month', b.start_date)";
        break;
      case 'week':
        dateGrouping = "DATE_TRUNC('week', b.start_date)";
        break;
      default:
        dateGrouping = "DATE(b.start_date)";
    }
    ...
    let query = `
      SELECT 
        ...
        COALESCE(SUM(CASE WHEN b.status = 'approved' THEN b.total_price ELSE 0 END), 0) as total_revenue,
        COALESCE(SUM(CASE WHEN b.status = 'approved' THEN b.price_deposit ELSE 0 END), 0) as total_deposits
      FROM bookings b
      WHERE b.field_id = $1
    `;
