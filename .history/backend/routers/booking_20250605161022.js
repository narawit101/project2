...
WHERE b.user_id = u.user_id
  ...
  AND (
    NOW() > b.updated_at + INTERVAL '1 minutes'
    OR (
       ...
       WHERE b.booking_id = $1`,
        ...
        if (userInfo) {
          ...
          <p>การจองสนาม <strong>${userInfo.field_name}</strong> ของคุณได้รับการอนุมัติแล้ว</p>
            <a href="${process.env.FONT_END_URL}/login?redirect=/bookingDetail/${booking_id}">ดูรายละเอียด</a>
          <p>(กรุณาแนบสลิปมัดจำ(ถ้ามี) ภายใน 1 ชม. หลังจากอนุมัติ)</p>
        `;
          } else if (booking_status === "rejected") {
            subject = "การจองของคุณไม่ได้รับการอนุมัติ";
            message = `
  <p>การจองสนาม <strong>${userInfo.field_name}</strong> ของคุณไม่ได้รับการอนุมัติ</p>
  <p>
    <a href="${process.env.FONT_END_URL}/login?redirect=/bookingDetail/${booking_id}">
      คลิกเพื่อดูรายละเอียดการจอง
    </a>
  </p>
`;
          }
  ...
  async (req, res) => {
    ...
    try {
      const bookingId = req.params.booking_id;
      const totalSlip = req.files["total_slip"]?.[0]?.path?.replace(/\\/g, "/");
      ...
      if (result.rows.length === 0) {
        return res.status(404).json({
          success: false,
          message: "ไม่พบข้อมูล payment สำหรับ booking นี้",
        });
      }
