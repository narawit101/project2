...
router.put("/resent-reset-password", async (req, res) => {
  const { email } = req.body;
  try {
    ...
    // ลบ OTP เก่าที่เก็บไว้ในฐานข้อมูลก่อนที่จะสร้าง OTP ใหม่
    await pool.query("DELETE FROM password_reset WHERE user_id = $1", [user_id]);

    // ฟังก์ชันในการสร้าง OTP ใหม่
    function generateNumericOtp(length) {
      const otp = crypto.randomBytes(length).toString("hex").slice(0, length); // สร้าง OTP
      return otp;
    }
