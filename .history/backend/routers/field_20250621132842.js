...
const storage = new CloudinaryStorage({
  ...
  params: async (req, file) => {
    ...
    return {
      folder: folder,
      resource_type: "auto", // auto = อัปโหลดได้ทั้งรูป, pdf, etc.
      format: file.mimetype.split("/")[1], // เช่น jpg, png, pdf
...
router.post(
  "/register",
  upload.fields([{ name: "documents", maxCount: 10  }, { name: "img_field" , maxCount: 1 }]),
  authMiddleware,
  async (req, res) => {
    try {
      const {
