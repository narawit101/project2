...
router.post("/facilities/:field_id", async (req, res) => {
  const { field_id } = req.params;
  ...
  try {
    ...
    for (const facId in selectedFacilities) {
      ...
      const result = await pool.query(
        `INSERT INTO field_facilities (field_id, facility_id, fac_price) 
         VALUES ($1, $2, $3)
         RETURNING field_fac_id`,
        [field_id, facId, facPrice]
      );
      insertedFacilities.push(result.rows[0]); // { field_fac_id: xxx }
    }

    res.status(200).json({ message: "บันทึกสำเร็จ", inserted: insertedFacilities });
  } catch (error) {
...
// ตัวอย่าง API ที่ใช้ field_fac_id เพื่อระบุสิ่งอำนวยความสะดวกที่ต้องการลบ
router.delete("/field/facilities/:field_id/:field_fac_id", async (req, res) => {
  const { field_id, field_fac_id } = req.params;
  console.log("Trying to delete field_fac_id:", field_fac_id, "for field_id:", field_id);
  ...
  try {
    ...
    if (result.rowCount === 0) {
      return res.status(404).json({ message: "ไม่พบสิ่งอำนวยความสะดวกนี้ในสนาม" });
    }
