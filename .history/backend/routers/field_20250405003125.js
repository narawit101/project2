...
router.delete("/delete/field/:id", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    await client.query("DELETE FROM posts WHERE field_id = $1", [fieldId]);
     ...
     if (documents) {
      // แปลงเป็น array ถ้าเป็น JSON string หรือ array
      let docPaths = [];
    
      try {
        if (typeof documents === "string") {
          // สำหรับเคสที่ documents เป็น JSON array ใน string เช่น '["uploads/documents/file.pdf"]'
          docPaths = JSON.parse(documents);
        } else if (Array.isArray(documents)) {
          docPaths = documents;
        } else {
          docPaths = [documents]; // fallback
        }
      } catch (err) {
      ...
      for (const doc of docPaths) {
        const docPath = path.resolve(__dirname, "..", doc.replace(/\\/g, "/")); // replace backslash
        if (fs.existsSync(docPath)) {
          try {
            fs.unlinkSync(docPath);
          } catch (e) {
            console.error("Error deleting document file:", e);
          }
        }
      }
    }
