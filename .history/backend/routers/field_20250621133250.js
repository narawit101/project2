...
const storage = new CloudinaryStorage({
  ...
  params: async (req, file) => {
    ...
    return {
      ...
      public_id: `${Date.now()}_${Math.random().toString(36).substr(2, 9)}`, // เพิ่ม random เพื่อป้องกันชื่อไฟล์ซ้ำ
    };
  },
});
...
const upload = multer({
  storage: storage,
  limits: {
    files: 11, // เพิ่มจาก 10 เป็น 11 (documents: 10 + img_field: 1)
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // แก้ไขการจัดการ documents
      let documents = [];
      if (req.files["documents"] && req.files["documents"].length > 0) {
        documents = req.files["documents"]
          .map((file) => {
            // ใช้ secure_url จาก Cloudinary แทน path
            return file.path || file.secure_url;
          })
          .filter(path => path) // กรองค่า null/undefined
          .join(", ");
      }

      const imgField = req.files["img_field"] && req.files["img_field"].length > 0
        ? (req.files["img_field"][0].path || req.files["img_field"][0].secure_url)
        : null;
      ...
      if (!user_id || !field_name) {
        return res.status(400).send({ error: "ข้อมูลผู้ใช้หรือชื่อสนามไม่ครบถ้วน" });
      }
      ...
      // INSERT ข้อมูลสนามย่อย (เพิ่มการตรวจสอบ)
      ...
      // INSERT ข้อมูลสิ่งอำนวยความสะดวก (เพิ่มการตรวจสอบ)
      if (selectedFacilities && typeof selectedFacilities === 'object') {
      ...
      if (userData.rows.length === 0) {
        return res.status(404).send({ error: "ไม่พบข้อมูลผู้ใช้" });
      }
      ...
      if (userEmail) {
        try {
          ...
          // ไม่ return error เพราะการลงทะเบียนสำเร็จแล้ว
        }
      }
      ...
      res.status(200).send({ 
        ...
        field_id,
        documents_count: req.files["documents"] ? req.files["documents"].length : 0
      });
      ...
      if (error instanceof SyntaxError) {
        return res.status(400).send({ error: "ข้อมูล JSON ไม่ถูกต้อง" });
      }
      
      if (error.code === '23505') { // PostgreSQL unique violation
        return res.status(400).send({ error: "ข้อมูลซ้ำในระบบ" });
      }
...
async function deleteCloudinaryFile(fileUrl) {
  try {
    ...
    const fileWithExt = urlParts.pop(); // เช่น 1750483997582.png
    const folder = urlParts.pop(); // เช่น field-profile
    const filename = fileWithExt.split(".")[0]; // ตัด .png ออก
    const ext = fileWithExt.split(".")[1]; // png, pdf, ico, etc.

    const publicId = `${folder}/${filename}`;

    // เลือก resource_type ตามนามสกุล
    let resourceType = "image";
    if (["pdf", "doc", "docx", "txt", "zip", "ico"].includes(ext.toLowerCase())) {
      resourceType = "raw";
    }
    ...
    if (result.result === "ok") {
      ...
      console.warn(`⚠️ ลบไม่สำเร็จ หรือไฟล์ไม่เจอ: ${publicId}`, result);
    }
  } catch (error) {
  ...
  try {
    ...
    if (documents) {
      ...
      for (const doc of docPaths) {
        await deleteCloudinaryFile(doc);
      }
    }
