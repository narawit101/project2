...
router.delete("/facilities/item", async (req, res) => {
  const { field_id, facility_id, fac_price } = req.body;

  try {
    const result = await pool.query(
      `DELETE FROM field_facilities 
       WHERE field_id = $1 AND facility_id = $2 AND fac_price = $3`,
      [field_id, facility_id, fac_price]
    );
