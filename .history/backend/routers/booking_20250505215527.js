    ...
    const overlapResult = await client.query(
      `SELECT * FROM bookings
        WHERE sub_field_id = $1
          ...
          AND (
            (
              -- กรณีเวลาปกติ (start_time < end_time)
              start_time < end_time
              AND (CAST($2 || ' ' || start_time AS timestamp) < CAST($4 AS timestamp))
              AND (CAST($2 || ' ' || end_time AS timestamp) > CAST($3 AS timestamp))
            )
            OR
            (
              -- กรณีเวลาข้ามวัน (เช่น 23:30 - 00:00)
              start_time >= end_time
              AND (CAST($2 || ' ' || start_time AS timestamp) < (CAST($4 AS timestamp) + INTERVAL '1 day'))
              AND ((CAST($2 || ' ' || end_time AS timestamp) + INTERVAL '1 day') > CAST($3 AS timestamp))
            )
            OR
            (
              -- กรณีเวลา 00:00 ซึ่งจะต้องเป็นการข้ามวัน
              start_time = '00:00' AND CAST($2 || ' 00:00' AS timestamp) = CAST($4 AS timestamp)
            )
          )`,
      [
