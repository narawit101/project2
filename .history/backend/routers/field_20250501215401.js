...
router.delete("/delete/addon/:id", authMiddleware, async (req, res) => {
  ...
  if (isNaN(addOnId) || !Number.isInteger(Number(addOnId))) {
    return res.status(400).json({ error: "Invalid Add-on ID" });
  }

  try {
    // ตรวจสอบว่ามี Add-on นี้จริงก่อน
    const check = await pool.query(
      "SELECT * FROM field_add_on WHERE add_on_id = $1",
      [addOnId]
    );
    ...
    // ลบ add-on
    await pool.query("DELETE FROM field_add_on WHERE add_on_id = $1", [
      addOnId,
    ]);
